...
static void refresh_telemetry(bool play,RV player){
    ...
    for(size_t i=0;i<sizeof(stat_keys)/sizeof(*stat_keys);i++){
        if(i)tj(",");const char* key=stat_keys[i];
        // Player fields for needs can be stale defaults (live Fatigue was 0
        // while characterDataMap was 2.26). The C-panel map is authoritative.
        double current=character_stat(key);
        if(!isfinite(current)){RV value=live_member(player,key);current=number(value);release_value(&value);}
        // b* fields are not universally baselines: bmax_hp is a zero bonus,
        // and player defaults can differ from the actual C-panel calculation.
        double baseline=!strcmp(key,"Weapon_Damage")?100:NAN;
    ...
    for(int i=0;count%4==0&&i+3<count&&i<1024;i+=4){
        if(telemetry_at>sizeof(telemetry_buffer)-24000){sources_truncated=true;break;}
    ...
    tj("],\"foodsComplete\":");tj(foods_complete?"true":"false");append_journey(player);tj("}");if(telemetry_full)shared->telemetry[0]=0;else memcpy(shared->telemetry,telemetry_buffer,telemetry_at+1);
}
#include "fodder.h"
