...
static void reconcile_speed(void){
    ...
    if(GetTickCount64()>heartbeat+1500){reasons|=8;preferred=1;auto_center=false;highlight_requested=false;}
...
static void on_command(void) {
    ...
    else if(cmd==CMD_RESET){finish_walk(WALK_MANUAL,true);preferred=1;auto_center=false;highlight_requested=false;set_speed(baseline);restore_camera(false);publish(0,"Normal speed and camera restored");}
    ...
    else if(cmd==14){int result=(!isfinite(arg)||floor(arg)!=arg||arg<0||arg>5)?4:start_walk((int)arg);publish(result,result?"Native walk unavailable":"Native map journey requested");}
...
__declspec(dllexport) DWORD WINAPI BridgeStart(void* ignored) {
    ...
    wchar_t name[128];swprintf(name,128,L"Local\\StoneshardCompanion.v5.%lu",GetCurrentProcessId());
