...
public sealed class StatsWindow:Window
{
    ...
    public StatsWindow(MainWindow parent){
        owner=parent;Title="晶石助手 · 角色属性";Width=290;Height=380;MinWidth=250;MinHeight=200;
        ...
        var root=new DockPanel();Content=surface=new Border{BorderBrush=new SolidColorBrush(Color.FromRgb(112,98,71)),CornerRadius=new CornerRadius(8),Child=root};
        ApplyAppearance();
    ...
    private void Rebuild(){
        ...
        foreach(var group in visible.GroupBy(s=>s.Group)){
            ...
            foreach(var definition in group){
                ...
                var explanation=new TextBlock{MaxWidth=440,TextWrapping=TextWrapping.Wrap};row.ToolTip=explanation;
                row.ToolTipOpening+=(_,_)=>explanation.Text=snapshot.Fresh(Environment.TickCount64)?snapshot.Explain(definition):"数据已过期，等待游戏更新。";
