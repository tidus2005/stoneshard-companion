...
public enum EngineCommand : uint { Refresh=1,Speed=2,Center=3,Player=4,Visor=5,Reset=6,Diagnostic=7,AutoCenter=8,Suspend=9,Inspect=10,Drink=11,Torch=12,Highlight=13,Walk=14,HighlightSet=15 }
    ...
    bool SceneReady,ulong SceneGeneration,ulong WindowGeneration,double PreferredMultiplier,uint SuspendReasons,uint UiFlags,double Hunger,double Thirst,double Pain,double Intoxication,double GuiWidth,double GuiHeight,uint VitalValid,int HighlightState,int TorchState,int WaterUses,long UpdatedAt,bool Ready)
{
    ...
    public string WalkStatus => WalkState switch {
        1=>WalkDirection==5?"正在走向地图中心 · 点击方向键停步":WalkPhase==1?"正在跨入相邻地图 · 点击方向键停步":$"正在前往{(WalkDirection==1?"上":WalkDirection==2?"下":WalkDirection==3?"左":"右")}边缘 · 点击方向键停步",
        2=>"已到达地图中心",3=>"行走已停止 · 手动接管",4=>"行走已停止 · 敌人或受伤",5=>"行走已停止 · 原版面板打开",
        6=>"地图变化或加载 · 本次行走结束",7=>"寻路中断、目标或出口不可达",8=>"行走已停止 · 游戏在后台",9=>"行走已停止 · 长时间无进展",_=>"方向键跨入相邻地图 · 中心键走到地图中央"
    };
}
...
public sealed class EngineBridge : IDisposable
{
    ...
    private EngineBridge(GameSession session,MemoryMappedFile map,FileStream ownership)
    {
        ...
        if(view.ReadUInt32(0)!=Native.Magic || view.ReadUInt32(4)!=5 || view.ReadInt32(8)!=session.Pid || view.ReadInt64(16)!=session.Started){view.Dispose();throw new InvalidDataException("游戏连接校验失败。");}
    ...
    public static async Task<EngineBridge> ConnectAsync(GameSession session,string library,CancellationToken token=default)
    {
        ...
        string name=$"Local\\StoneshardCompanion.v5.{session.Pid}";
        foreach(int version in new[]{1,2,3,4})try{using var old=MemoryMappedFile.OpenExisting($"Local\\StoneshardCompanion.v{version}.{session.Pid}",MemoryMappedFileRights.Read);throw new NotSupportedException("游戏仍加载旧版助手组件，请在方便时正常退出游戏并重新启动一次。");}catch(FileNotFoundException){}
    ...
    public static EngineState DecodeSnapshot(byte[] bytes)
    {
        ...
        return new(I(64),BitConverter.ToUInt32(bytes,68),D(72),D(80),D(88),D(96),D(104),D(112),D(120),D(128),D(136),D(144),D(152),I(160),I(164),BitConverter.ToUInt64(bytes,176),S(184,512),S(696,3072),I(3768)==1,
            ...
            {SupplyFlags=(uint)I(3896),TorchCount=I(3900),TorchDuration=D(3904),HighlightApplied=I(3912)==1,WalkState=(uint)I(3916),WalkDirection=I(3920),WalkPhase=(uint)I(3960),LabelHookReady=I(3924)==1,WalkX=D(3928),WalkY=D(3936),LabelDrawCalls=BitConverter.ToUInt64(bytes,3944),LabelDrawOverrides=BitConverter.ToUInt64(bytes,3952)};
    }
