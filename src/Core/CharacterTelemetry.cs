...
public sealed class CharacterTelemetry
{
    ...
    public string Explain(StatDefinition definition){
        ...
        b.AppendLine();b.AppendLine("游戏记录的来源：");
        var sources=Sources.Where(s=>s.Key==definition.Key).ToArray();
        foreach(var source in sources)b.AppendLine($"• {(string.IsNullOrWhiteSpace(source.Label)?$"效果来源 #{source.SourceId:0}":source.Label)}：{(source.Delta>=0?"+":"")}{Format(source.Delta,definition.Unit=="%"?" 个百分点":definition.Unit)}");
        ...
        b.AppendLine();b.AppendLine("基础值仅显示游戏明确提供的数值。来源列表可能包含装备、技能与状态；乘算、上限、取整及未记录来源不作猜测，列表不保证可直接相加还原最终值。");
