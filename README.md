...
从源码下载后，请先按下方“开发”步骤构建。构建完成后双击根目录 `启动晶石助手.cmd`，或运行 `artifacts/release/StoneshardCompanion-0.3.3/StoneshardCompanion.exe`。新版使用独立目录，保留旧版成品。EXE、桥接 DLL 和 Assets 文件夹须一起保留；升级后需重启游戏以载入新桥接组件。

- 单面板：拖动顶部移动，拖动四角调整宽高，按钮自适应排列，记住位置和尺寸。
- 物品常显：移动和装备栏切换时持续显示地面物品名称，切图及重连后恢复开启状态。
- 十字导航：左侧固定上下左右和中心五键；四向到边缘后继续跨入相邻地图，中心键让角色走到地图中央。遇敌、受伤或手动接管时停止，不自动攻击。
...
0.3.3 按用户要求仅修改代码、运行离线检查并构建发布，未启动游戏。新增自动跨图、中心行走和常驻面板尚未实机验证。旧版已验证项目和未覆盖项见历史报告；自动火把点亮及耗尽换备用仍未实机验证。
